...
> Provides a simple CLI which allows the user to manually call team_shop_outbot.py by providing the path for the desired file.
> May not be needed once more progress has been made.

team_shop_outbot.py
> Parses the contents of an Order Folder and the Count .CSV contained therein, then prepares one or more emails with appropriate text and attachments.
