...
def main(outbound_list):
    ...
    # For clarity:
    # The outbound_list is a list of HTAs that have been ordered by the customer.
    # The inbound_list is a list of HTAs that we don't already have in the warehouse, which must be made or ordered.
    ...
    for i in range(len(outbound_list)):
        ...
        if units_needed > 0:
            try:
                units_on_hand = int(cursor.execute('SELECT "Units on Hand" FROM Inventory WHERE Name=? AND "HTA Number"=?',
