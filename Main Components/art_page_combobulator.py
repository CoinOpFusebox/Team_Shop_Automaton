...
# reach out to me!
#
# Fair warning, this thing is slow. Glacial. Especially if you're pulling HTAs from the server. I hope to optimize it
# somewhat at some point, but at least it's functional.
#
...
def main(inbound_list, team_name, store_number, film_mode, page_number):
    ...
    for order_hta in inbound_list:
        ...
        # I'm sure it's still not perfect, but I've tested it on about 300 random, individual HTAs and also run about
        # fifty art sheets through it without incident, so it should work most of the time.
        ...
        # I didn't find out about ExecuteMenuCommand() until very late in the process, might have been useful elsewhere?
...
def combobulate(folder_path):
    ...
    for file in os.listdir(folder_path):
        if file.endswith('.csv'):
            ...
            transfer_list = count_comparitron.main(count_transmogrifier.main(csv_path, False))
    ...
    try:
        if transfer_list:
            if len(transfer_list) <= 10:
                main(transfer_list, team_name, store_number, False, 0)
                ...
                for sublist in transfer_iter:
                    main(sublist, team_name, store_number, False, page_number)
                    page_number += 1
    ...
    try:
        if film_list:
            if len(film_list) <= 10:
                main(film_list, team_name, store_number, True, 0)
                ...
                for sublist in film_iter:
                    main(sublist, team_name, store_number, True, page_number)
                    page_number += 1
