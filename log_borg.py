...
def main(folder_path):
            ...
            if 'Heat Transfer' in str(line_x):
                ...
                for line_y in reader:
                    if 'Total' in str(line_y):
                        ...
                        if info_dump:
                            print('Total Art HTAs: ' + heat_transfer_total)
                        break
                break

        if has_heat_transfers == 'No':
            if info_dump:
                print('Total Art HTAs: 0')
        ...
        if has_player_numbers == 'Yes':
            for line in reader:
                if '# Total' in str(line):
                    ...
                    if info_dump:
                        print('Total Number HTAs: ' + player_number_total)
                    break
        else:
            if info_dump:
                print('Total Number HTAs: 0')
            ...
            if 'Film' in str(line_x):
                for line_y in reader:
                    if 'Total' in str(line_y):
                        film_total = line_y[1]
                        if info_dump:
                            print('Total Film HTAs: ' + film_total)
                        break
                break

        if film_total == 0:
            if info_dump:
                print('Total Film HTAs: 0')
    ...
    if info_dump:
        print('HTA Visual Link: ' + hta_visual_link + '\n')
